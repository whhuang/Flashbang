...
let importMenuOpen = false, exportMenuOpen = false;
...
document.getElementById('top-right-btn').addEventListener('click', () => {
  ...
  document.getElementsByClassName('new-bunch-container')[0].classList.add(
      'hide');
  ...
  if (e.key === 'Enter') {
    if (document.activeElement.id != 'import-text') {
      ...
      if (document.activeElement.id == 'title-input') {
        ...
        activePair = (document.querySelector('input.prompt:focus') ||
                      document.querySelector('input.answer:focus'))
                         .parentNode.parentNode.querySelector('p.index')
                         .innerText -
            1;
      }
...
function parseImport() {
  try {
    ...
    for (x = 0; x < pairsStrings.length; x++) {
      ...
      const pair = {};
      pair['prompt'] =
          pairArray[0].trim();  // removed leading and trailing white spaces
    ...
    document.getElementById('import-valid-message').classList.remove('hide');
    clearTimeout(importTimeout);
    importTimeout = setTimeout(() => {
      document.getElementById('import-valid-message').classList.add('hide');
    }, 3000);
  }
}
...
function closeImportMenu() {
  ...
  document.getElementsByClassName('new-bunch-container')[0].classList.remove(
      'hide');
...
function updateExport() {
  ...
  for (x = 0; x < pairs.length - 1; x++) {
    textArea.value +=
        `${pairs[x].prompt}${termSeparator}${pairs[x].answer}${pairSeparator}`;
  }
  textArea.value += `${pairs[x].prompt}${termSeparator}${pairs[x].answer}`;
}

document.getElementById('info-icon-export')
...
document.getElementById('btwn-pair-export')
    .addEventListener('change', updateExport);
document.getElementById('btwn-term-export')
    .addEventListener('change', updateExport);
...
function removeBlanks() {
  ...
  for (x = 0; x < oglength; x++) {
    if (pairs[x - splicedCount].prompt === '' &&
...
function makePairs() {
  const htmlPairs =
      document.getElementById('pair-container').getElementsByClassName('pair');
